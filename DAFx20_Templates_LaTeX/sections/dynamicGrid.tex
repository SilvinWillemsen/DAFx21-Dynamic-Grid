...
\SBcomment[OK, here is where things start to get nasty. You have $\delta_{tt}u_{l_{u}}^{n}$ appearing. But really this should be $\delta_{tt}u_{l_{u}^{n}}^{n}$, shouldn't it? And then how does the operator $\delta_{tt}$ work in this case? Like, if $l_{u}^{n}$ is changing from one time step to the next?] \SWcomment[see email and comment at the beginning of Section \ref{sec:systSetup}.]
...
Although the correction presented here is not based on some physical process, it can be justified by the fact that large differences in displacement between two spatially adjacent points is not physical. \SWcomment[$\leftarrow$ tried to justify]
