...
Algorithm \ref{alg:calcOrder} shows the order of calculation to implement the method presented in this paper. Important is to only retrieve a change in $c$ at time index $n$ before all other calculations, so that $u_l^n$ and $w_l^n$ are calculated with the same $\alpha$ and $\beta$ time index $n$ for all $l$.
\begin{algorithm}[ht]
    \setstretch{1.25}
