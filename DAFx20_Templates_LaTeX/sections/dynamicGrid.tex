...
Consider two grid functions, $u_\lu^n$ and $w_\lw^n$ defined over discrete domains $\lu = [0, \hdots, M]$ and $\lw = [0, \hdots, M_w]$ respectively with integers $M = \lceil 0.5L/ck\rceil $ with $\lceil \cdot \rceil$ denoting the ceiling operation and $M_w = \lfloor 0.5L/ck\rfloor$, i.e., half the number of points allowed by the stability condition, plus one for overlap. The grid functions can then be placed in line with each other with $u_M^n$ and $w_0^n$ (defined as the inner boundaries) overlapping, i.e., $x_{u_M}^n = x_{w_0}^n$, and $u_0^n$ and $w_{M_w}^n$ (defined as the outer boundaries) located at $x_{u_0} = 0$ and $x_{w_{M_w}} = L$. See Figure \ref{fig:twoFreeStrings}. The following boundary conditions are then imposed:
